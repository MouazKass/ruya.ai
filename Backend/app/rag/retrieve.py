...
class Retriever:
    ...
    def retrieve(
        ...
        from_clickhouse = self._search_clickhouse_cases(case_id=case_id, query_embedding=query_embedding, k=k)
        from_past = self.past_outbreak_index.search(query_embedding=query_embedding, k=k)
        ...
        return {
            "clickhouse_records": self.reranker.rerank(query_text, from_clickhouse, top_k=k),
            "past_outbreak_cases": self.reranker.rerank(query_text, from_past, top_k=k),
            "strategy_memory": self.reranker.rerank(query_text, from_strategy, top_k=k),
            "query_case": {
                "case_id": case_id,
    ...
    def _search_clickhouse_cases(self, case_id: str, query_embedding: list[float], k: int) -> list[dict[str, Any]]:
        limit = int(self.settings.max_vector_scan)
        rows = self.clickhouse.query_dicts(
            f"""
            SELECT case_id, run_id, normalized_json, embedding, created_at
            FROM cases
